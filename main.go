...
func (t Training) TrainingInfo() InfoMessage {
	var infoM InfoMessage
	infoM.TrainingType = t.TrainingType
	infoM.Duration = t.Duration
	infoM.Distance = t.distance()
	infoM.Speed = t.meanSpeed()
	infoM.Calories = t.Calories()
	return infoM
}
...
func (w Walking) Calories() float64 {
	speedInM := w.meanSpeed() * KmHInMsec
	if w.Height == 0 {
		return 0
	}
	calories := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedInM, 2)/w.Height/CmInM)*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration.Hours()) * MinInHours)
	return calories
}
...
func (s Swimming) meanSpeed() float64 {
	speedSwim := float64(s.LengthPool*s.CountPool) / MInKm / float64(s.Duration.Hours())
	return speedSwim
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	var infoM InfoMessage
	infoM.TrainingType = s.TrainingType
	infoM.Duration = s.Duration
	infoM.Distance = s.distance()
	infoM.Speed = s.meanSpeed()
	infoM.Calories = s.Calories()
	return infoM
}
